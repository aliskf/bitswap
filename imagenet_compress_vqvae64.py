...
from model.imagenet_train_vqvae64 import Model
from torch.utils.data import *
from discretization_vqvae64 import *
...
class ANS:
    ...
    def encode(self, x, symbols):
        for i, s in enumerate(symbols):
            ...
            if x[-1] >= ((self.lbound >> self.bits) << 64) * pmf:
                x.append(x[-1] >> 64)
...
def compress(quantbits, nz, bitswap, gpu):
    ...
    for ei in range(experiments):
        ...
        for xi in iterator:
            ...
            # calculating bits
            totaladdedbits = (len(state) - len(initialstate)) * 64
            totalbits = (len(state) - (len(restbits) - 1)) * 64

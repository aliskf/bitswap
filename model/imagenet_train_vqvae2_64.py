...
class Model(nn.Module):
    def __init__(self,in_channels: int, embedding_dim: int, num_embeddings: int, hidden_dims: List = None, img_size: int = 64, xs=(3, 64, 64), nz=1, zchannels=16, nprocessing=1, kernel_size=3, resdepth=2,
                 reswidth=256, dropout_p=0., tag='', root_process=True, beta: float = 0.25,
                  **kwargs):
        super().__init__()
        ...
        self.zdim = (self.zchannels, 32, 32)
        ...
        if root_process:
            ...
            log_dir = os.path.join(
                'runs/imagenet64/', current_time + '_' + socket.gethostname() + tag)
        ...
        self.infer_in = nn.Sequential(
            # shape: [1,64,64] -> [4,16,16]
            modules.Squeeze2d(factor=2),

            # shape: [4,16,16] -> [64,16,16]
        ...
        self.deepvq_layer = nn.ModuleList(VectorQuantizer(num_embeddings,
                                            embedding_dim,
                                            self.beta)
    ...
    def infer(self, i):
        ...
        def distribution(given):
            ...
            # if compressing, the input might not be float64, so we'll have to convert it first
            if self.compressing:
                type = h.type()
                h = h.float()
    ...
    def generate(self, i):
        ...
        def distribution(given):
            ...
            # if compressing, the input is flattened, so we'll have to convert it back to a Tensor
            # also, the input might not be float64, so we'll have to convert it first
            if self.compressing:
                type = h.type()
                h = h.float()
                h = h.view((-1,) + self.zdim)
    ...
    def loss(self, x):
        ...
        #logvq = torch.mean(logvq, dim=1) * self.bitsscale
        logvq = torch.mean(logvq) * self.bitsscale
        logrecon = torch.mean(logrecon) * self.bitsscale
        return logrecon, logdec, logenc, zsamples, logvq
    ...
    def reconstruct(self, x_orig, device, epoch):
        # take only first 64 datapoints of the input
        # otherwise the output image grid may be too big for visualization
        x_orig = x_orig[:64, :, :, :].to(device)
...
def warmup(model, device, data_loader, warmup_batches, root_process):
    ...
    for batch_idx, image in enumerate(data_loader):        # stack image with to current stack
    ...
    with modules.init_mode():
        logrecon, logdec, logenc, _ , logvq = model.loss(warmup_images)
    ...
    if root_process:
        ...
        model.logger.add_scalar('x/vq/train', entvq,0)
        ...
        for i in range(1, logdec.shape[0] + 1):
            ...
            #model.logger.add_scalar(f'z{i}/vq/train', entvq[i - 1],0)
...
def train(model, device, epoch, data_loader, optimizer, ema, log_interval, root_process, schedule=True, decay=0.99995):
    ...
    for batch_idx, batch in enumerate(data_loader):        # keep track of the global step
        ...
        logrecon, logdec, logenc, zsamples, logvq = model.loss(data)
        ...
        if root_process and batch_idx % log_interval == 0 and log_interval < nbatches:
            ...
            model.logger.add_scalar('x/vq/train', entvq, global_step)
            ...
            for i in range(1, logdec.shape[0] + 1):
                ...
                #model.logger.add_scalar(f'z{i}/vq/train', entvqz[i - 1], global_step)
...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    # enumerate over the batches
    for batch_idx, batch in enumerate(data_loader):        # save batch for reconstruction
    ...
    if root_process:
        ...
        entvq= -torch.mean(logvqs).detach().cpu().numpy()
        entvqz= -torch.mean(logvqsz, dim=0).detach().cpu().numpy()
        ...
        for i in range(1, logdec.shape[0] + 1):
            ...
            model.logger.add_scalar(f'z{i}/vq/test',entvqz[i - 1], epoch)
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'model/params/imagenet64/'):
                os.makedirs(f'model/params/imagenet64/')
            torch.save(model.state_dict(), f'model/params/imagenet64/{tag}')
            if epoch % 25 == 0:
                torch.save(model.state_dict(), f'model/params/imagenet64/epoch{epoch}_{tag}')
            print("saved params\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--zchannels', default=16, type=int, help="number of channels for the latent variables")
    parser.add_argument('--nprocessing', default=8, type=int, help='number of processing layers')
    ...
    train_set = modules.ImageNet(root='data/imagenet64/train', file='train.npy', transform=transform_ops)

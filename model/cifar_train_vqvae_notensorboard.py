...
class Model(nn.Module):
    def __init__(self,in_channels: int, embedding_dim: int, num_embeddings: int, hidden_dims: List = None, img_size: int = 32, xs=(3, 32, 32), nz=1, zchannels=16, nprocessing=1, kernel_size=3, resdepth=2,
...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    if root_process:
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'model/params/cifar/'):
                os.makedirs(f'model/params/cifar/')
            torch.save(model.state_dict(), f'model/params/cifar/{tag}')
            if epoch % 25 == 0:
                torch.save(model.state_dict(), f'model/params/cifar/epoch{epoch}_{tag}')
            print("saved params\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch', default=384, type=int, help="size of the mini-batch for gradient descent")
    ...
    parser.add_argument('--continue_training', default=0, type=int, help="continue training")
    ...
    kwargs = {'num_workers': 10, 'pin_memory': True} if distributed else {}
    ...
    if continue_training == 1:
        ...
        ckp_path = ("f'model/params/cifar/{tag}")

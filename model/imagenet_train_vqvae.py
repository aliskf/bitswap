...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    if root_process:
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'model/params/imagenet/'):
                os.makedirs(f'model/params/imagenet/')
            torch.save(model.state_dict(), f'model/params/imagenet/{tag}')
            if epoch % 25 == 0:
                torch.save(model.state_dict(), f'model/params/imagenet/epoch{epoch}_{tag}')
            print("saved params\n")

...
from model.cifar_train_vqvae import Model
from torch.utils.data import *
from discretization_vqvae import *
...
def compress(quantbits, nz, bitswap, gpu):
    ...
    # save experiments
    np.save(f"plots/cifar{nz}/{'bitswap' if bitswap else 'bbans'}_{quantbits}bits_nets",nets)
    np.save(f"plots/cifar{nz}/{'bitswap' if bitswap else 'bbans'}_{quantbits}bits_elbos", elbos)
    np.save(f"plots/cifar{nz}/{'bitswap' if bitswap else 'bbans'}_{quantbits}bits_cmas",cma)
    np.save(f"plots/cifar{nz}/{'bitswap' if bitswap else 'bbans'}_{quantbits}bits_total", total)
